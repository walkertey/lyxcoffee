...
export function CartVendorGroup({ vendorName, lines, subtotal, onChangeQty, onUpdateNote }: CartVendorGroupProps) {
  return (
    <div className="bg-[rgba(255,253,248,0.94)] border-t border-[var(--line)] first:border-t-0">
      {/* Vendor Header */}
      <div className="px-4 py-[14px] bg-[linear-gradient(90deg,rgba(169,22,22,0.10),rgba(201,154,52,0.13))] text-[#7F1010] font-black flex items-center justify-between gap-3">
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                maxLength={80}
                className="mt-2 w-full min-h-[38px] border border-[var(--line)] rounded-[14px] px-2.5 py-2 bg-[var(--paper)] text-[var(--ink)] outline-none resize-y text-[13px] focus:border-[rgba(169,22,22,0.42)] focus:shadow-[0_0_0_4px_rgba(169,22,22,0.08)]"
            ...
            <div className="grid md:grid-cols-1 grid-flow-col md:grid-flow-row gap-2 justify-items-start md:justify-items-end items-start md:items-center justify-between md:justify-start">
              <div className="text-[#7F1010] font-black">{centsToMoney(getCartItemTotalCents(line))}</div>
              <QuantityControl
                quantity={line.qty}
